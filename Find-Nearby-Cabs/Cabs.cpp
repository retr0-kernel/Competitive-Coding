...
double GreatCircleDistance(double lat2d, double lon2d){
    double lat1, lon1, lat2, lon2, delta_lon, central_ang;
    lat1 = degreetoradian(lat1d);
    lon1=degreetoradian(lon1d);
    lat2 = degreetoradian(lat2d);
    lon2 = degreetoradian(lon2d);
...
struct json{
    long long int length, i, j, x, y, m, n, f, fi, id[1000000];
    ...
    void distance_calculator(){
        if (GreatCircleDistance(lat2d, lon2d) <= 50.0000)
        {
